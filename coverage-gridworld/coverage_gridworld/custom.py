...
def _obs_a(grid: np.ndarray) -> np.ndarray:
    ...
    return channels.flatten()  # (600,)


def _obs_space_a() -> gym.spaces.Space:
    return gym.spaces.Box(low=0.0, high=1.0, shape=(600,), dtype=np.float32)
...
def _reward_fn3(info: dict) -> float:
    ...
    r -= adjacent_danger * 2.0

    # Small per-step penalty to encourage speed (tiebreaker metric)
    r -= 0.05
